...
(html/defsnippet role-option (str globals/template-path "admin.html") [(html/attr= :field "role_option")]
                 ...
                 [(html/attr= :field "role_option")] (html/do->
                                                       ...
                                                       (html/content (name option))
                                                                ;(when (= true selected) (html/set-attr :selected selected))
                                                                  ))
...
(html/defsnippet admin-user-table-content (str globals/template-path "admin.html") [(html/attr= :field "table-entry")]
                 ...
                 ;[(html/attr= :field "role-td")] (html/content (map #(str (name %)
                 ;                                                        (when (= (name %) (name (db/role-kw user))) "yea selected"))
                 ;                                                   db/available-roles))
                 [(html/attr= :field "role-select")] (html/content (map
                                                                     #(role-option %
                                                                                    (if (= (name (db/role-kw user)) (name %))
                                                                                      "true"
                                                                                      "false")
                                                                                    ;)
                                                                                    ) db/available-roles))
